...
int main(int argc, const char ** argv) {
    ...
    arg_list args;
    args.add_argument(string_arg("--model-path", "(REQUIRED) The local path of the Kokoro GGUF model file.", "-mp", true));
    ...
    args.add_argument(bool_arg("--vad", "(OPTIONAL) whether to apply voice inactivity detection (VAD) and strip silence from the end of the output. By default, no VAD is applied.", "-va"));
    ...
    args.add_argument(int_arg("--max-tokens", "(OPTIONAL) The max audio tokens or token batches to generate where each represents approximately 11 ms of audio. If set to zero (default), uses the model's default max generation size. Warning: values under 15 are not supported.", "-mt", false, &default_max_tokens));
