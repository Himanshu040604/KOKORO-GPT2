...
int main(int argc, const char ** argv) {
    ...
    arg_list args;
    args.add_argument(string_arg("--model-path", "(REQUIRED) The local path of the Kokoro GGUF model file to quantize.", "-mp", true));
    ...
    args.add_argument(bool_arg("--quantize-text-embedding", "(OPTIONAL) Whether to quantize the input text embeddings. Defaults to false and is true when passed (does not accept a parameter).", "-qe"));
    args.add_argument(bool_arg("--quantize-cross-attn-kv", "(OPTIONAL) Whether to quantize the cross attention keys and values. Defaults to false and is true when passed (does not accept a parameter).", "-qkv"));
