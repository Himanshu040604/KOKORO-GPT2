...
// Language code to eSpeak voice ID mapping
static std::map<char, const char*> KOKORO_LANG_TO_ESPEAK_ID = 
{
    {'a', "gmw/en-US"}, {'b', "gmw/en"}, {'e', "roa/es"},
    {'f', "roa/fr"}, {'h', "inc/hi"}, {'i', "roa/it"},
    {'j', "jpx/ja"}, {'p', "roa/pt-BR"}, {'z', "sit/cmn"}
};

// LSTM cell with forward and reverse weights for bidirectional processing
struct lstm_cell 
{
...
// LSTM layer with hidden states and cells
struct lstm 
{
...
{
    lstm* rnn = nullptr;
    ggml_tensor* ada_norm_gamma_weight = nullptr;
    ggml_tensor* ada_norm_gamma_bias = nullptr;
    ggml_tensor* ada_norm_beta_weight = nullptr;
    ggml_tensor* ada_norm_beta_bias = nullptr;
};

// Residual conv block with adaptive normalization
struct ada_residual_conv_block 
{
    ...
    ggml_tensor* norm1_gamma = nullptr;
    ggml_tensor* norm1_gamma_bias = nullptr;
    ggml_tensor* norm1_beta = nullptr;
    ggml_tensor* norm1_beta_bias = nullptr;
    ggml_tensor* norm2_gamma = nullptr;
    ggml_tensor* norm2_gamma_bias = nullptr;
    ggml_tensor* norm2_beta = nullptr;
    ggml_tensor* norm2_beta_bias = nullptr;
    ggml_tensor* pool = nullptr;  // Optional pooling layer
    ggml_tensor* pool_bias = nullptr;
    ggml_tensor* upsample = nullptr;  // Optional upsampling layer
    ggml_tensor* upsample_bias = nullptr;
};

// Predicts duration, F0 (pitch), and energy curves
struct duration_predictor 
{
    ggml_tensor* albert_encode = nullptr;
    ggml_tensor* albert_encode_bias = nullptr;
    std::vector<duration_predictor_layer*> layers;
    lstm* duration_proj_lstm = nullptr;
    ggml_tensor* duration_proj = nullptr;
    ggml_tensor* duration_proj_bias = nullptr;
    ggml_tensor* n_proj_kernel = nullptr;  // Energy projection
    ggml_tensor* n_proj_bias = nullptr;
    ggml_tensor* f0_proj_kernel = nullptr;  // Pitch projection
    ggml_tensor* f0_proj_bias = nullptr;
    lstm* shared_lstm = nullptr;
    std::vector<ada_residual_conv_block*> f0_blocks;  // Pitch refinement
    std::vector<ada_residual_conv_block*> n_blocks;   // Energy refinement
};
...
// Encodes phoneme tokens to acoustic features
struct kokoro_text_encoder 
{
    ggml_tensor* embd = nullptr;
    std::vector<kokoro_text_encoder_conv_layer*> conv_layers;
    lstm* out_lstm = nullptr;
};

// Multi-receptive field residual block for HiFi-GAN generator
...
{
    uint32_t input_conv_stride = 0;
    uint32_t input_conv_padding = 0;
    ggml_tensor* input_conv = nullptr;
    ggml_tensor* input_conv_bias = nullptr;
    kokoro_generator_residual_block* res_block = nullptr;
};
...
struct kokoro_generator_upsample_block 
{
    uint32_t padding = 0;
    uint32_t stride = 0;
    ggml_tensor* upsample_weight = nullptr;  // Transposed conv1d
    ggml_tensor* upsample_bias = nullptr;
};
...
struct kokoro_generator 
{
    ...
    ggml_tensor* m_source_weight = nullptr;
    ggml_tensor* m_source_bias = nullptr;
    ggml_tensor* out_conv_weight = nullptr;
    ggml_tensor* out_conv_bias = nullptr;
...
// Combines acoustic features and generates mel-spectrogram
struct kokoro_decoder 
{
    ggml_tensor* f0_conv = nullptr;         // Pitch convolution
    ggml_tensor* f0_conv_bias = nullptr;
    ggml_tensor* n_conv = nullptr;          // Energy convolution
    ggml_tensor* n_conv_bias = nullptr;
    ggml_tensor* asr_conv = nullptr;        // Acoustic feature convolution
    ggml_tensor* asr_conv_bias = nullptr;
    std::vector<ada_residual_conv_block*> decoder_blocks;
    ada_residual_conv_block* encoder_block = nullptr;
    kokoro_generator* generator = nullptr;
};

// ALBERT transformer layer for text encoding
struct albert_layer 
{
    ...
    ggml_tensor* layer_output_norm_weight = nullptr;
    ggml_tensor* layer_output_norm_bias = nullptr;
    ggml_tensor* q = nullptr;  // Query projection
    ggml_tensor* k = nullptr;  // Key projection
    ggml_tensor* v = nullptr;  // Value projection
    ggml_tensor* o = nullptr;  // Output projection
...
struct kokoro_model : tts_model 
{
    ...
    uint32_t max_duration_per_token = 20;  // Increase for slower speech
    uint32_t style_half_size = 128;
    
    // Text encoder
    uint32_t n_conv_layers = 3;
    
    // Decoder and generator config
    ...
    ggml_tensor* n_kernels_tensor = nullptr;
    
    bool uses_pooling = false;
    bool static_token_types = true;
    
    std::map<std::string, ggml_tensor*> voices;

    // ALBERT embeddings
    ggml_tensor* embd_hidden = nullptr;
    ggml_tensor* embd_hidden_bias = nullptr;
    ...
    // Static computation tensors
    ggml_tensor* harmonic_sampling_norm = nullptr;
    ggml_tensor* sampling_factor_scalar = nullptr;
    ggml_tensor* sqrt_tensor = nullptr;
    
    // Model components
    duration_predictor* prosody_pred = nullptr;
    kokoro_text_encoder* text_encoder = nullptr;
    kokoro_decoder* decoder = nullptr;
    ...
    void assign_lstm(lstm* rnn, std::string_view name, ggml_tensor* tensor);
    void assign_generator_weight(kokoro_generator* generator, std::string_view name, ggml_tensor* tensor);
    void assign_gen_resblock(kokoro_generator_residual_block* block, std::string_view name, ggml_tensor* tensor);
    void assign_ada_res_block(ada_residual_conv_block* block, std::string_view name, ggml_tensor* tensor);
    void assign_decoder_weight(std::string_view name, ggml_tensor* tensor);
    void assign_duration_weight(std::string_view name, ggml_tensor* tensor);
    void assign_text_encoder_weight(std::string_view name, ggml_tensor* tensor);
    void assign_albert_weight(std::string_view name, ggml_tensor * tensor);
    ...
    void setup_from_file(gguf_context* meta_ctx, ggml_context* load_context, bool cpu_only = true) {
        std::function<void(ggml_tensor*)> fn = ([this](ggml_tensor* cur) 
        {
...
// Forward declaration for duration response
struct kokoro_duration_response;

// Batch of tokens for processing
struct kokoro_ubatch 
{
    size_t n_tokens = 0;
    uint32_t* input_tokens = nullptr;
    kokoro_duration_response* resp = nullptr;
};

// Context for duration prediction
struct kokoro_duration_context : runner_context 
{
    kokoro_duration_context(kokoro_model* model, int n_threads) 
...
// Forward declarations for graph building functions
static ggml_tensor* build_albert_attn_mask(ggml_context* ctx, kokoro_duration_context* kctx, const kokoro_ubatch& batch);
...
static kokoro_generator_upsample_block* build_upsample_block_from_file(gguf_context* meta, int index);

const char* get_espeak_id_from_kokoro_voice(std::string_view voice);
kokoro_duration_context* build_new_duration_kokoro_context(kokoro_model* model, int n_threads, bool use_cpu = true);

// Response from duration prediction
struct kokoro_duration_response 
{
    size_t n_outputs = 0;
...
// Manages graph computation for duration prediction
struct kokoro_duration_runner : tts_runner 
{
    kokoro_duration_runner(kokoro_model* model, kokoro_duration_context* context, single_pass_tokenizer* tokenizer)
        : model(model), kctx(context), tokenizer(tokenizer) {}
    ...
    single_pass_tokenizer* tokenizer = nullptr;
    kokoro_model* model = nullptr;
    kokoro_duration_context* kctx = nullptr;
...
// Context for main generation
struct kokoro_context : runner_context 
{
    kokoro_context(kokoro_model* model, int n_threads)
        : runner_context(n_threads), model(model) {}
    ...
    ggml_tensor* inp_tokens = nullptr;
    ggml_tensor* duration_pred = nullptr;
    ggml_tensor* duration_mask = nullptr;
    ggml_tensor* window_sq_sum = nullptr;
    ggml_tensor* uv_noise_data = nullptr;
...
// Generator and vocoder graph builders
static ggml_tensor* build_generator(ggml_context* ctx, kokoro_model* model, kokoro_context* kctx, 
                                     ggml_tensor* x, ggml_tensor* style, ggml_tensor* f0_curve, 
                                     kokoro_generator* generator, int sequence_length, 
                                     ggml_tensor* window_sq_sum, ggml_cgraph* gf);
static ggml_tensor* build_sin_gen(ggml_context* ctx, kokoro_model* model, kokoro_context* kctx, 
                                   ggml_tensor* x, int harmonic_num, int sequence_length, 
...
// Main TTS runner for generation
struct kokoro_runner : tts_generation_runner 
{
    kokoro_runner(unique_ptr<kokoro_model> model, kokoro_context* context, single_pass_tokenizer* tokenizer, 
                  kokoro_duration_runner* drunner, phonemizer* phmzr, const generation_configuration& config)
        : tts_generation_runner{kokoro_loader}, 
          model{move(model)}, 
          kctx(context), 
          tokenizer(tokenizer), 
          drunner(drunner), 
    ...
    kokoro_context* kctx = nullptr;
    kokoro_duration_runner* drunner = nullptr;
    phonemizer* phmzr = nullptr;
...
// Model implementation - graph building functions
...
static ggml_tensor* build_albert_inputs(ggml_context* ctx, kokoro_model* model, ggml_tensor* input_tokens, ggml_tensor* positions, ggml_tensor* token_types) 
{
    ...
    ggml_tensor* out = ggml_cont(ctx, build_albert_norm(ctx, ainpts, model->input_norm_weight, model->input_norm_bias));
    return ggml_add(ctx, ggml_mul_mat(ctx, model->embd_hidden, out), model->embd_hidden_bias);
}

static ggml_tensor* build_albert_norm(ggml_context* ctx, ggml_tensor* cur, ggml_tensor* weight, ggml_tensor* bias) 
{
    const float eps = 0.000000000001f; // Epsilon for Albert normalization
    cur = ggml_norm(ctx, cur, eps);
    cur = ggml_cont(ctx, ggml_add(ctx, ggml_mul(ctx, cur, weight), bias));
    return cur;
}

// Forward declaration for LSTM helper
static ggml_tensor* build_lstm_run(ggml_context* ctx, ggml_cgraph* gf, ggml_tensor* input, ggml_tensor* h_0, ggml_tensor* c_0, 
                                    std::vector<ggml_tensor*> weights, std::vector<ggml_tensor*> biases, 
                                    uint32_t sequence_length, bool reversed = false);

// Build LSTM layer with optional bidirectional processing
static ggml_tensor* build_lstm(ggml_context* ctx, ggml_tensor* input, lstm* rnn, uint32_t sequence_length, ggml_cgraph* gf) 
{
...
// Adaptive residual convolution block
static ggml_tensor* build_ada_residual_conv(ggml_context* ctx, ggml_tensor* x, ada_residual_conv_block* block, ggml_tensor* style, ggml_tensor* sqrt_tensor) 
{
    ggml_tensor* cur = x;
    ...
    cur = ggml_norm(ctx, cur, 0.00001f);
    cur = ggml_add(ctx, cur, ggml_mul(ctx, cur, ggml_transpose(ctx, gamma)));
    cur = ggml_add(ctx, cur, ggml_transpose(ctx, beta));
    cur = ggml_leaky_relu(ctx, cur, 0.2f, false);
...
// Generator residual block with adaptive normalization
static ggml_tensor* build_kokoro_generator_res_block(ggml_context* ctx, ggml_tensor* x, ggml_tensor* style, kokoro_generator_residual_block* block) 
{
    ...
    for (int i = 0; i < block->convs1_weights.size(); i++) {
        ...
        cur = ggml_cont(ctx, ggml_transpose(ctx, ggml_norm(ctx, inpl, 0.00001f)));
        ...
        cur = ggml_cont(ctx, ggml_transpose(ctx, ggml_norm(ctx, cur, 0.00001f)));
...
// Sinusoidal generator for harmonic synthesis
...
{
    ggml_tensor* cur = ggml_mul(ctx, ggml_repeat(ctx, x, ggml_new_tensor_2d(ctx, GGML_TYPE_F32, x->ne[0], harmonic_num)), model->harmonic_sampling_norm);
    ...
    // ggml doesn't support bool tensors or greater-than/roll ops
    // Using 1.0 and 0.0 for booleans with custom multiplications

    ggml_tensor* uv_noise = ggml_map_custom3(ctx, fake, upscaled, kctx->uv_noise_data, &uv_noise_compute, sequence_length, nullptr);
    ggml_tensor* noise = ggml_cont(ctx, ggml_view_2d(ctx, uv_noise, uv_noise->ne[0], uv_noise->ne[1], uv_noise->nb[1], uv_noise->nb[2]));
    ggml_tensor* uv = ggml_cont(ctx, ggml_view_2d(ctx, uv_noise, uv_noise->ne[0], uv_noise->ne[1], uv_noise->nb[1], 0));
    return ggml_cont(ctx, ggml_transpose(ctx, ggml_add(ctx, ggml_mul(ctx, ggml_sin(ctx, cur), uv), noise)));
}

// Main generator function with STFT/iSTFT processing
...
{
    ggml_tensor* sing = build_sin_gen(ctx, model, kctx, f0_curve, model->harmonic_num + 1, f0_curve->ne[0] * 300, model->voice_threshold, model->sin_amp, model->noise_std);
    ggml_tensor* har = ggml_tanh(ctx, ggml_add(ctx, ggml_mul_mat(ctx, generator->m_source_weight, sing), generator->m_source_bias));

    har = stft(ctx, ggml_cont(ctx, ggml_transpose(ctx, har)), generator->window, model->true_n_fft, model->stft_hop, true, true);
    // STFT gives magnitude and phase separately
    // Split and concat them in the needed format

    ggml_tensor* mhar = ggml_cont(ctx, ggml_view_3d(ctx, har, har->ne[0], har->ne[1], har->ne[2], har->nb[1], har->nb[2], 0));
    ggml_tensor* phhar = ggml_cont(ctx, ggml_view_3d(ctx, har, har->ne[0], har->ne[1], har->ne[2], har->nb[1], har->nb[2], har->nb[3]));
    ...
    for (int i = 0; i < generator->ups.size(); i++) 
    {
        ...
        {
            ggml_tensor* temp = ggml_cont(ctx, ggml_view_3d(ctx, cur, 1, cur->ne[1], cur->ne[2], cur->nb[1], cur->nb[2], cur->nb[0]));
            cur = ggml_concat(ctx, temp, cur, 0);
        }
        ggml_tensor* x_source = build_noise_block(ctx, generator->noise_blocks[i], ggml_cont(ctx, combined_har), style);
        cur = ggml_add(ctx, cur, x_source);
        ggml_tensor* x_inner = cur;
        for (int ii = 0; ii < model->n_kernels; ii++) {
            if (ii == 0) {
                cur = build_kokoro_generator_res_block(ctx, x_inner, style, generator->res_blocks[i * model->n_kernels + ii]);
            } else {
                cur = ggml_add(ctx, cur, build_kokoro_generator_res_block(ctx, x_inner, style, generator->res_blocks[i * model->n_kernels + ii]));
            }
        }
    ...
    ggml_tensor* phase = ggml_view_3d(ctx, cur, cur->ne[0], cur->ne[1] - model->post_n_fft, cur->ne[2], cur->nb[1], cur->nb[2], cur->nb[1] * model->post_n_fft);
    phase = ggml_sin(ctx, phase);
    spec = ggml_exp(ctx, spec);
    cur = ggml_concat(ctx, spec, phase, 3);
    // iSTFT needs magnitude and phase concatenated after batch dimension
...
// Factory function to build residual block from file metadata
static kokoro_generator_residual_block* build_res_block_from_file(gguf_context* meta, std::string_view base_config_key) 
{
    kokoro_generator_residual_block* grb = new kokoro_generator_residual_block;
    // Residual blocks always have 3 convolutional layers
    for (int i = 0; i < 3; i++) {
        ...
        const std::string key_base = std::string(base_config_key);
        int padding_key = gguf_find_key(meta, (key_base + "." + std::to_string(i) + ".padding").c_str());
        int dilation_key = gguf_find_key(meta, (key_base + "." + std::to_string(i) + ".dilation").c_str());
        if (padding_key == -1 || dilation_key == -1) {
            TTS_ABORT("Could not find dilation and padding for generator residual block at key, '%s.%d'.", key_base.c_str(), i);
        }
...
static kokoro_noise_residual_block* build_noise_block_from_file(gguf_context* meta, int index)
{
    ...
    const std::string base = "kokoro.decoder.generator.noise_blocks." + std::to_string(index);
...
static kokoro_generator_upsample_block* build_upsample_block_from_file(gguf_context* meta, int index) 
{
    kokoro_generator_upsample_block* usb = new kokoro_generator_upsample_block;
    const std::string base = "kokoro.decoder.generator.up_convs." + std::to_string(index);
    int stride_key = gguf_find_key(meta, (base + ".stride").c_str());
    int padding_key = gguf_find_key(meta, (base + ".padding").c_str());
    if (padding_key == -1 || stride_key == -1) {
...
// Initialize post-load tensors and constants
...
// Assign LSTM weights from file tensor
void kokoro_model::assign_lstm(lstm* rnn, std::string_view name, ggml_tensor* tensor) 
{
    std::vector<std::string> parts = split(std::string(name), ".");
...
void kokoro_model::assign_weight(const char* name, ggml_tensor& tensor) 
{
    ...
    {
        assign_albert_weight(name_sv.substr(sizeof("albert.") - 1), &tensor);
    } 
    else if (name_sv.starts_with("duration_predictor.")) 
    {
        assign_duration_weight(name_sv.substr(sizeof("duration_predictor.") - 1), &tensor);
    } 
    else if (name_sv.starts_with("text_encoder.")) 
    {
        assign_text_encoder_weight(name_sv.substr(sizeof("text_encoder.") - 1), &tensor);
    } 
    else if (name_sv.starts_with("decoder.")) 
    {
        assign_decoder_weight(name_sv.substr(sizeof("decoder.") - 1), &tensor);
    } 
...
void kokoro_model::assign_generator_weight(kokoro_generator* generator, std::string_view name, ggml_tensor* tensor) 
{
    ...
    {
        std::vector<std::string> parts = split(string(name), ".");
        ...
        if (parts[0] == "noise_blocks") {
            if (parts[2] == "conv_weight") {
                ...
                assign_gen_resblock(generator->noise_blocks[i]->res_block, string(name).substr(parts[0].size() + parts[1].size() + parts[2].size() + 3), tensor);
            }
        } else if (parts[0] == "resblocks") {
            assign_gen_resblock(generator->res_blocks[i], string(name).substr(parts[0].size() + parts[1].size() + 2), tensor);
        } else if (parts[0] == "ups") {
            if (parts[2] == "weight") {
...
// Helper: Squeeze a 3D tensor with first dimension equal to 1 into a 2D tensor
...
void kokoro_model::assign_ada_res_block(ada_residual_conv_block* block, std::string_view name, ggml_tensor* tensor)
{
...
void kokoro_model::assign_decoder_weight(std::string_view name, ggml_tensor* tensor)
{
    ...
    } else if (has_prefix(string(name), "decoder_blocks")) {
        std::vector<std::string> parts = split(string(name), ".");
        int i = std::stoi(parts[1]);
        assign_ada_res_block(decoder->decoder_blocks[i], parts[2], tensor);
    } else if (has_prefix(string(name), "encoder_block")) {
        std::vector<std::string> parts = split(string(name), ".");
        assign_ada_res_block(decoder->encoder_block, parts[1], tensor);
    } else if (has_prefix(string(name), "generator")) {
        assign_generator_weight(decoder->generator, string(name).substr(10), tensor);
    }
}

void kokoro_model::assign_duration_weight(std::string_view name, ggml_tensor* tensor)
{
    if (name == "encode") {
        ...
        if (parts[0] == "shared_lstm") {
            assign_lstm(prosody_pred->shared_lstm, string(name).substr(parts[0].size() + 1), tensor);
        } else if (parts[0] == "duration_lstm") {
            assign_lstm(prosody_pred->duration_proj_lstm, string(name).substr(parts[0].size() + 1), tensor);
            ...
            if (parts[2] == "gamma_weight") {
                ...
                assign_lstm(prosody_pred->layers[i]->rnn, string(name).substr(parts[0].size() + parts[1].size() + parts[2].size() + 3), tensor);
            }
        }
    }
}

void kokoro_model::assign_gen_resblock(kokoro_generator_residual_block* block, std::string_view name, ggml_tensor* tensor) 
{
    std::vector<std::string> parts = split(std::string(name), ".");
    int i = std::stoi(parts[0]);
...
// Initialize a bidirectional LSTM cell with 8 weight/bias pairs
...
void kokoro_model::prep_layers(gguf_context* meta)
{
    ...
    for (int i = 0; i < n_upsamples; i++) {
        kokoro_generator_upsample_block* ub = build_upsample_block_from_file(meta, i);
        decoder->generator->ups.push_back(ub);
    }

    for (int i = 0; i < n_res_blocks; i++) {
        kokoro_generator_residual_block* rb = build_res_block_from_file(meta, "kokoro.decoder.generator.res_blocks." + std::to_string(i));
...
// 110000 is the graph size for the longest sequence.
// Precomputing it is hard since nodes don't align with tensors or submodels.
// So the graph size was calculated once and set as a constant.
ggml_cgraph* kokoro_duration_runner::build_kokoro_duration_graph(kokoro_ubatch& batch)
{
    ...
    for (int r = 0; r < model->n_recurrence; r++) {
        for (int l = 0; l < model->n_layers; l++) {
            ...
            // Self-attention
            {
                ggml_tensor* Qcur = ggml_add(ctx, ggml_mul_mat(ctx, model->layers[l]->q, cur), model->layers[l]->q_bias);
                ggml_tensor* Kcur = ggml_add(ctx, ggml_mul_mat(ctx, model->layers[l]->k, cur), model->layers[l]->k_bias);
                ggml_tensor* Vcur = ggml_add(ctx, ggml_mul_mat(ctx, model->layers[l]->v, cur), model->layers[l]->v_bias);
                ...
                ggml_tensor* v = ggml_cont_3d(ctx, ggml_transpose(ctx, Vcur), batch.n_tokens, model->head_size, model->n_attn_heads);
    ...
    ggml_tensor* style_half = ggml_cont(ctx, ggml_view_1d(ctx, voice, voice->ne[0] / 2, voice->ne[0] / 2 * voice->nb[0] + (batch.n_tokens - 3) * voice->nb[1]));
    ...
    for (auto l : model->prosody_pred->layers) {
        ...
        ggml_tensor* gamma = ggml_add(ctx, ggml_mul_mat(ctx, l->ada_norm_gamma_weight, style_half), l->ada_norm_gamma_bias);
        ggml_tensor* beta = ggml_add(ctx, ggml_mul_mat(ctx, l->ada_norm_beta_weight, style_half), l->ada_norm_beta_bias);

        cur = ggml_norm(ctx, cur, 0.00001);

        // ggml can't multiply by scalars directly, so gamma * x and x are added here.
        // A simple fix is to add 1 to gamma when making the gguf file.
        cur = ggml_add(ctx, ggml_add(ctx, cur, ggml_mul(ctx, cur, gamma)), beta);
        cur = ggml_concat(ctx, cur, ggml_repeat(ctx, style_half, ggml_new_tensor_2d(ctx, GGML_TYPE_F32, style_half->ne[0], cur->ne[1])), 0);
    }
    ...
    // To support speed, add a constant speed tensor and divide here.
    ggml_tensor* len = nullptr;
...
// Kokoro skips causal attention since it's not an autoregressive model
void kokoro_duration_runner::set_inputs(kokoro_ubatch& batch)
{
    ...
    for (uint32_t i = 0; i < batch.n_tokens; i++) {
        ...
        for (uint32_t ii = 0; ii < batch.n_tokens; ii++) {
            attn_d[i * batch.n_tokens + ii] = 0.0f;
        }
    }
}
...
void kokoro_duration_runner::run(kokoro_ubatch& batch)
{
    ...
    // The output is the last tensor in the graph
    // Reused hidden states are computed before each node chunk, whose size depends on sequence length
    ...
    // Reset the state before syncing so the CPU reset work can run while the device is computing
    ggml_backend_sched_reset(kctx->sched);
...
// 570000 is the graph size for the longest sequence.
// Precomputing it is tricky since nodes don't match tensors directly.
// So the size was calculated once and set as a constant.
ggml_cgraph* kokoro_runner::build_kokoro_graph(kokoro_ubatch& batch)
{
    ...
    ggml_tensor* voice = model->voices[kctx->voice];
    ggml_tensor* style_half = ggml_view_1d(ctx, voice, voice->ne[0] / 2, voice->ne[0] / 2 * voice->nb[0] + (batch.n_tokens - 3) * voice->nb[1]);
    ggml_tensor* cur = nullptr;
    ...
    ggml_tensor* asr_res = nullptr;
    ggml_tensor* f0 = nullptr;
    ggml_tensor* n_base = nullptr;
...
void kokoro_runner::run(kokoro_ubatch& batch, tts_response& outputs)
{
    ...
    // Reset state before syncing so CPU work can run alongside device computation
    ggml_backend_sched_reset(kctx->sched);
...
// tokenize_chunks splits long prompts (over 512 tokens) by sentence or nearest space for cleaner speech
std::vector<std::vector<uint32_t>> kokoro_runner::tokenize_chunks(std::vector<std::string> clauses)
{
    ...
    for (auto clause : clauses) {
        ...
        // If clause tokens exceed max length, split by spaces
        // If no spaces, split mid-word as fallback

        if (tokens.size() > model->max_context_length - 2) {
            // Skip the first token because it's the beginning-of-sequence marker
...
void kokoro_runner::propagate_voice_setting()
{
    ...
    // If the language changes, update the phonemization voice
...
void kokoro_runner::generate(const char* prompt, tts_response& response, const generation_configuration& config)
{
    ...
    espeak_voice_id = config.espeak_voice_id;
    propagate_voice_setting();
    // Replace non-sentence characters with '--' for espeak pauses
    // Keep other punctuation for cleaner chunking before tokenization

    std::string normalized{prompt};
    normalized = replace_any(prompt, ",;:", "--");
    normalized = replace_any(prompt, "\n", " ");
    std::string phonemized_prompt = phmzr->text_to_phonemes(normalized);
    // Kokoro uses a UTF-8 single-character tokenizer
    // If the prompt is short enough (without BOS and EOS tokens), compute it all at once

    if (phonemized_prompt.size() < model->max_context_length - 2) {
        // Keep punctuation, but remove it for all-at-once compute since Kokoro sees them as end-of-sentence
        ...
        // Chunking is likely slower, but may save memory with Kokoro's large graph
...
// Text encoder weight assignment
void kokoro_model::assign_text_encoder_weight(std::string_view name, ggml_tensor* tensor)
{
    if (name == "embedding_weight") {
        text_encoder->embd = ggml_dup_tensor(ctx, tensor);
        set_tensor(text_encoder->embd, tensor);
    } else if (has_prefix(string(name), "lstm")) {
        assign_lstm(text_encoder->out_lstm, string(name).substr(5), tensor);
    } else if (has_prefix(string(name), "layers")) {
        std::vector<std::string> parts = split(string(name), ".");
        int i = std::stoi(parts[1]);
        if (parts[2] == "gamma") {
...
void kokoro_model::assign_albert_weight(std::string_view name, ggml_tensor* tensor)
{
    if (name == "embd") {
    ...
    } else if (has_prefix(string(name), "layer")) {
        std::vector<std::string> parts = split(string(name), '.');
        int i = std::stoi(parts[1]);
        if (parts[2] == "ffn") {
